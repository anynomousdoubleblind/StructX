...
void print_token_array_as_bytes(const char* label, uint32_t* d_array, size_t length_uint32) {
    ...
    for (size_t i = 0; i < length_bytes && i < 200; i++) {
...
void print_sorted_tokens_after_depth_sort(
    const uint32_t* d_sorted_indices,
    const uint8_t* d_sorted_values,
    size_t count
) {
    print_uint32_indices("Sorted Token Indices (by depth)", d_sorted_indices, count);
    print_token_info("Sorted Token Values (by depth)", d_sorted_indices, d_sorted_values, count);
}
...
// // Functor to build “head flags” from segment-key array.
// template <typename KeyIter>
// struct is_segment_head_functor {
//     KeyIter keys;
//     __host__ __device__
...
// // Build a head-flag array from keys[0..n)
// template <typename KeyIter>
// void build_head_flags(KeyIter keys, bool* d_head_flags, size_t n) {
//     thrust::transform(
//         thrust::cuda::par,
//         thrust::make_counting_iterator<size_t>(0),
//         thrust::make_counting_iterator<size_t>(n),
//         d_head_flags,
//         is_segment_head_functor<KeyIter>{keys}
//     );
// }

// // Generic inclusive scan by key using CUB's segmented scan
// template <typename KeyIter, typename ValueIter>
// void inclusive_scan_by_key_cub(KeyIter keys, ValueIter values, int* d_output, size_t n) {
//     // Step 1: Allocate head flags
//     bool* d_head_flags = nullptr;
//     cudaMalloc(&d_head_flags, sizeof(bool) * n);

//     // Step 2: Build head flags
//     build_head_flags(keys, d_head_flags, n);

//     // Step 3: Temporary storage allocation
//     void* d_temp_storage = nullptr;
//     size_t temp_storage_bytes = 0;
